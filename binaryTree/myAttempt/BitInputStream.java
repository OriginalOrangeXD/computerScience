...
public final class BitInputStream implements AutoCloseable {
    ...
    public static char read() {
        if (isEmpty()) throw new NoSuchElementException("Reading from empty input stream");// special case when aligned byte
        if (currentByte == 7) {
            int x = buffer;
            fillBuffer();
            return (char) (x & 0xff);
        ...
        int x = buffer;
        x <<= (7 - numBitsRemaining);
        int oldN = numBitsRemaining;
        fillBuffer();
        if (isEmpty()) throw new NoSuchElementException("Reading from empty input stream");
        numBitsRemaining = oldN;
        x |= (buffer >>> numBitsRemaining);
        return (char) (x & 0xff);
